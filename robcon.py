...
def construct_knn_graph_from_clusters(seq, n_clusters):
    # seq 的形状是 (1, N, D)，需要去掉第一个维度变成 (N, D)
    seq = seq.squeeze(0)  # 变成 (N, D)
    # 将 seq 从 GPU 移动到 CPU 并转换为 numpy 数组
    seq_np = seq.cpu().detach().numpy()

    # 使用 K-means 聚类
    kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(seq_np)
    labels = kmeans.labels_

    # 构建聚类图（邻接矩阵）
    adj = torch.zeros((seq.size(0), seq.size(0)))
    ...
    return adj.to(seq.device)

def medoids_me(seq, adj, top_k, dim=1):
    adj_dense = adj.to_dense()
    # 计算每个节点的度数
    degrees = adj_dense.sum(dim=1)  # shape: (N,)

    # 选取度数最大的 top_k 个节点的索引
    top_k_indices = degrees.topk(top_k).indices  # shape: (top_k,)

    # 取出度最大的节点对应的特征向量
    seq_top_k = seq.index_select(dim, top_k_indices)

    # seq_top_k 的形状是 (1, N, D)
    # distances = torch.cdist(seq_top_k, seq_top_k, p=2)  # 计算所有点之间的欧氏距离
    distances = torch.cdist(seq, seq, p=2)  # 计算所有点之间的欧氏距离

    sum_distances = distances.sum(dim)  # 对给定维度求和
    medoid_indices = sum_distances.argmin(dim=dim)  # 找到距离和最小的索引
    medoids = seq.gather(dim, medoid_indices.unsqueeze(dim).unsqueeze(0).expand(-1, -1, seq.size(-1)))
    return medoids.squeeze(dim)
...
class DGI(nn.Module):
    ...
    def forward(self, seq1, seq2, seq3, seq4, adj, aug_adj1, aug_adj2, sparse, msk, samp_bias1, samp_bias2, aug_type):
        ...
        c_1 = self.read(h_1, adj, 10, msk)
        c_1 = self.sigm(c_1)

        c_3 = self.read(h_3, adj, 10, msk)
        c_3 = self.sigm(c_3)
...
class AvgReadout(nn.Module):
    ...
    def forward(self, seq, adj, top_k, msk):
        if msk is None:
            result = medoids_me(seq, adj, top_k, dim=1)
            # print("result: ", result)
